...
WHEN=2025-11-01;
HOW=React + TypeScript + Gemini AI integration
...
/**
 * Safely retrieves the Gemini API key from environment or localStorage
...
WHO=RapidWebDevelop; WHERE:D:\agentleegeminialmost\frontend\src\lib\planner\agent-lee-todo.ts; WHEN=2025-10-05;
...
// =================================================================================
// GEMINI SERVICE HOOK (for specialized Gemini features)
// =================================================================================
  ...
  const superiorPromptRef = useRef<string>('');
    ...
    const analyzeContent = async (capability: string, file: FileItem) => {
        try {
            ...
            switch(capability) {
                case 'Summarize': prompt = `Summarize the following content:\n\n${content}`; break;
                case 'Rewrite': prompt = `Rewrite the following content:\n\n${content}`; break;
                case 'Explain Code': prompt = `Explain the following code snippet:\n\n${content}`; break;
                case 'Describe Schema': prompt = `Describe the schema of the following JSON:\n\n${content}`; break;
                case 'Extract Text & Tables': prompt = `Extract the text and any tables from the following document content:\n\n${content}`; break;
                default: prompt = `Perform the following action on the content below: ${capability}\n\n${content}`;
            }

            const response = await gemini.models.generateContent({
                model: 'gemini-2.5-flash',
                contents: superiorPromptRef.current
                  ? `SYSTEM PERSONA INSTRUCTION:\n${superiorPromptRef.current.slice(0, 6000)}\n\nTASK:\n${prompt}`
                  : prompt,
            });
...
const AgentNotepad: React.FC = () => {
  ...
  const handleCapabilityClick = async (capability: string, file: FileItem) => {
    ...
    try {
      const result = await gemini.analyzeContent(capability, file);
      setAiResult(result ?? "");
    } catch (error) {
