...
export function validateAgentLeePersonaModule() {
  ...
  const hasAssetSuperior = fs.existsSync(path.join(root, "assets", "01_SUPERIOR_PROMPT", "Agent_Lee_Superior_Prompt.md"));
  const hasAssetManifest = fs.existsSync(path.join(root, "assets", "05_MANIFEST", "agentlee_persona_manifest.json"));
  const hasAssetHeritageMd = fs.existsSync(path.join(root, "assets", "06_HERITAGE", "agentlee_heritage_canon.md"));
  const hasAssetHeritageJson = fs.existsSync(path.join(root, "assets", "06_HERITAGE", "agentlee_heritage_canon.json"));

  const superiorPath = hasAssetSuperior
    ? path.join(root, "assets", "01_SUPERIOR_PROMPT", "Agent_Lee_Superior_Prompt.md")
    : path.join(root, "01_SUPERIOR_PROMPT", "Agent_Lee_Superior_Prompt.md");
  const manifestPath = hasAssetManifest
    ? path.join(root, "assets", "05_MANIFEST", "agentlee_persona_manifest.json")
    : path.join(root, "05_MANIFEST", "agentlee_persona_manifest.json");
  const heritageMdPath = hasAssetHeritageMd
    ? path.join(root, "assets", "06_HERITAGE", "agentlee_heritage_canon.md")
    : path.join(root, "06_HERITAGE", "agentlee_heritage_canon.md");
  const heritageJsonPath = hasAssetHeritageJson
    ? path.join(root, "assets", "06_HERITAGE", "agentlee_heritage_canon.json")
    : path.join(root, "06_HERITAGE", "agentlee_heritage_canon.json");
