...
```mermaid
graph TB
  subgraph Client_Layer["Frontend: Mission Command Center"]
    DASH["Secure Command Hub\n(React 19 + Framer Motion)"]
    VP["Premium HD Voice Pipeline\n(Multilingual + Emotion)"]
    ROI["Economic Moat Tracker\n(Savings vs Cloud)"]
    DASH --- VP
    DASH --- ROI
  end

  subgraph Network_Transport["Transport Protocol: High Fidelity RTC"]
    SFU["LeeWay Mediasoup SFU\n(C++ Optimized Workers)"]
    ICE["TURN/STUN Relay\n(Coturn)"]
    SFU <--> ICE
  end

  subgraph Intelligence_Tier["Guardian Core: NPC Agent Fleet"]
    direction TB
    tier1["Core Perception\n(ARIA, OBSERVER, VECTOR)"]
    tier2["Security & Ops\n(SENTINEL, WARD, NEXUS)"]
    tier3["Self-Healing Infra\n(REPAIR, SCALER)"]
    tier4["Oversight Tier\n(GOVERNOR)"]
    
    tier4 --> tier1 & tier2 & tier3
  end

  Client_Layer <-->|"Secure WebSockets + DTLS"| Network_Transport
  Network_Transport <--> Intelligence_Tier
```
